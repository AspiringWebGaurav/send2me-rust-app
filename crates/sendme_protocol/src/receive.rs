...
pub async fn download_to_store(
    ...
    let (stats, total_files, payload_size, rtt_ms) = if !local.is_complete() {
        ...
        // Capture RTT post-handshake before the connection is moved into execute_get.
        // Callers use this to pick storage-appropriate export concurrency and to
        // distinguish LAN (<5ms) from WAN paths for other tuning decisions.
        // PathId::ZERO is the default (only) path when multipath isn't negotiated.
        let rtt_ms = connection
        ...
        // Time-throttle UI progress to ~30 fps regardless of transfer speed.
        // A byte-based threshold fires thousands of times/sec on fast links
        // and swamps the Tauri IPC with format!()+serialize overhead.
        let mut last_emit_time = transfer_start_time;
        const PROGRESS_INTERVAL: Duration = Duration::from_millis(33);

        while let Some(item) = stream.next().await {
            match item {
                GetProgressItem::Progress(offset) => {
                    if last_emit_time.elapsed() >= PROGRESS_INTERVAL {
        ...
        (stats, total_files, payload_size, rtt_ms)
    } else {
        ...
        (Stats::default(), total_files, payload_bytes, 0u32)
    };
