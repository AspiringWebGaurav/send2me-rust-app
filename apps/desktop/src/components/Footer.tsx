...
export function Footer() {
  ...
  return (
    <motion.footer
      ...
      className="h-9 border-t border-border/60 bg-panel/75 backdrop-blur-xl flex items-center justify-between px-4 sm:px-6 z-50 shrink-0 text-[11px] font-medium tracking-wide"
    >
      <div className="flex items-center gap-3">
        <div className={`flex items-center gap-2 transition-colors ${isOnline ? 'text-foreground/80' : 'text-danger'}`}>
        ...
        <div className={`flex items-center gap-1.5 transition-colors ${isP2PActive ? 'text-muted-foreground' : 'text-danger/80'}`}>
          {isP2PActive ? (
            <Zap className="w-3 h-3 text-primary" />
          ) : (
            <WifiOff className="w-3 h-3 text-danger" />
          )}
          <span>{isP2PActive ? "P2P Active" : "P2P Inactive"}</span>
        </div>
      </div>

      <div className="items-center gap-3 text-muted-foreground/70 hidden md:flex">
        <button
      ...
      <div className="flex items-center gap-3 text-muted-foreground">
        ...
        >
          ...
          <span className="transition-colors group-hover:text-primary">GitHub</span>
        </button>
        <div className="w-px h-3 bg-border/70 hidden sm:block"></div>
        <span className="hidden sm:block tabular-nums">Rust Core v{appInfo?.version || "0.0.0"}</span>
      </div>
    </motion.footer>
  );
}
