...
pub const AUTH_LABEL: &[u8] = b"sendme-device-auth-v1";

/// Cap for control-plane JSON messages. Real payloads are < 1 KB in practice;
...
pub enum ControlMessage {
    ...
    FolderSyncBindRequest {
        device_name: String,
        os: String,
    },
    FolderSyncBindResponse {
        accepted: bool,
    },
    FolderSyncBindFinalize {
        accepted: bool,
    },
    FolderSyncUnbind {
        node_id: String,
...
pub async fn write_message(
    ...
    anyhow::ensure!(body.len() <= MAX_CONTROL_MESSAGE_SIZE, "control message too large");
...
pub async fn read_message(
    ...
    anyhow::ensure!(
        len > 0 && len <= MAX_CONTROL_MESSAGE_SIZE,
